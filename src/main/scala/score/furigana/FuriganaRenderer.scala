...
object FuriganaRenderer:
  ...
  def renderPNG(furiText: Iterable[(String, String)]): Option[Array[Byte]] =
    ...
    val totalWidth = positionedFuri.map(f => (f.furiWidth max f.textWidth) + f.x).maxOption.getOrElse(0)
    val totalHeight = positionedFuri.headOption.fold(0)(_.y) + lineHeight
